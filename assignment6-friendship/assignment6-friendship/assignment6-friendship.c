...
//user_bst prototypes
void swapUser(user_bst *, user_bst *);
user_bst * createUser(char *, char *);
user_bst * insertUser(user_bst *, char *, char *);
user_bst * removeUser(user_bst *, char *); // not sure if i need or not
...
int main()
{
    ...
    fptr = fopen("/Users/carolinasantos/src/computer-science-1/assignment6-friendship/assignment6-friendship/ex_big.txt","r");
    ...
    bptr = fopen("/Users/carolinasantos/src/computer-science-1/assignment6-friendship/assignment6-friendship/output2.txt","w");
    ...
    {
        fscanf(fptr, "%s", query);
        fscanf(fptr, "%s", firstName);
        
        fprintf(bptr, "%s %s", query, firstName);
            
        if (strcmp(query, "ADD") == 0)
        {
            fscanf(fptr, "%s", secondName);
            fprintf(bptr, " %s\n", secondName);
...
// delete later
void inOrderPrintUser(user_bst * rt)
{
    if (rt == NULL) return;
    inOrderPrintUser(rt->l);
    printf("%s\n", rt->name);
    inOrderPrintUser(rt->r);
}
...
int containsUser(user_bst * rt, char * name)
{
    ...
    printf(" inside contains user -> root name: %s\n name 1: %s\n", rt->name, name);
...
// Delete possibly
void swapUser(user_bst * a, user_bst * b)
{
...
// Delete possibly
user_bst * removeUser(user_bst * rt, char * name)
{
    if (rt == NULL)
    {
    ...
    {
        removeUser(rt->r, name);
        return rt;
    }
    else if (strcmp(rt->name, name) > 0)
    {
        removeUser(rt->l, name);
        return rt;
    }
    ...
    user_bst * replacement = rt->r;
    
    while (replacement->l != NULL)
        replacement = replacement->l;

    swapUser(replacement, rt);
    
    removeUser(replacement, name);
